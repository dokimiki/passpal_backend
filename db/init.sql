...
COMMENT ON TABLE users IS
  'アプリ利用ユーザー。Firebase UID や BAN 状態を保持する';
...
COMMENT ON TABLE user_devices IS
  'ユーザーと FCM デバイストークンの紐付け（複数デバイス対応）';
...
COMMENT ON TABLE notification_reports IS
  '課題通知をバックエンドへ報告した履歴';
...
COMMENT ON TABLE classes IS
  '学期+MaNaBo 授業ID 単位の授業情報（科目マスタと紐付く）';
...
COMMENT ON TABLE class_notifiers IS
  '授業ごとの通知対象ユーザーリスト';
...
COMMENT ON TABLE assignments IS
  '課題情報。MaNaBo 課題 ID と公開/締切日時・ステータスを保持する';
